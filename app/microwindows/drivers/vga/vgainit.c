...
#define	PALREG	0x3c0 /* VGA_AC_INDEX    */
#define	SEQREG	0x3c4 /* VGA_SEQ_INDEX   */
#define	SEQVAL	0x3c5 /* VGA_SEQ_DATA    */
#define	GRREG	0x3ce /* VGA_GC_INDEX    */
#define	GRVAL	0x3cf /* VGA_GC_DATA     */
#define	ATTRREG	0x3da /* VGA_INSTAT_READ */
#define	CRTCREG	0x3d4 /* VGA_CRTC_INDEX  */
#define	CRTCVAL	0x3d5 /* VGA_CRTC_DATA   */

#define	GENREG1	0x3c2 /* VGA_MISC_WRITE */
#define	GENREG2	0x3cc /* VGA_MISC_READ  */
...
static REGIO graph_off[] = {
  ...
  { OUT, GENREG1, 0x63, 0, 0 }, /* Control Register */

  /* Reset sequencer regs */

  { OUT, SEQREG, 0, SEQVAL, 1 },  /* Reset b0,1:Reset=0 */
  { OUT, SEQREG, 1, SEQVAL, 0 },  /* Clock Mode */
  { OUT, SEQREG, 2, SEQVAL, 3 },  /* Map Mask : Plane Write Enable */
  { OUT, SEQREG, 3, SEQVAL, 0 },  /* Character Generator Select */
  { OUT, SEQREG, 4, SEQVAL, 2 },  /* Memory Mode */
  ...
  { OUT, CRTCREG, 8, CRTCVAL, 0x00 },	/* row scan */
  { OUT, CRTCREG, 9, CRTCVAL, 0x4f },	/* max scan line */
  { OUT, CRTCREG, 10, CRTCVAL, 0x0d },	/* cursor start */
  { OUT, CRTCREG, 11, CRTCVAL, 0x0e },	/* cursor end */
  { OUT, CRTCREG, 12, CRTCVAL, 0x00 },	/* start high addr */
  { OUT, CRTCREG, 13, CRTCVAL, 0x00 },	/* low addr */
  { OUT, CRTCREG, 14, CRTCVAL, 0x00 },	/* cursor high */
  { OUT, CRTCREG, 15, CRTCVAL, 0x50 },	/* cursor low */
  { OUT, CRTCREG, 16, CRTCVAL, 0x9c },	/* vert retrace */
  { OUT, CRTCREG, 17, CRTCVAL, 0x0e },	/* retrace end */
  ...
  { OUT, GRREG, 0, GRVAL, 0x00 },  /* Set/Reset Color Code */
  { OUT, GRREG, 1, GRVAL, 0x00 },  /* Enable Set/Reset Latch */
  { OUT, GRREG, 2, GRVAL, 0x00 },  /* Color Compare */
  { OUT, GRREG, 3, GRVAL, 0x00 },  /* Data Rotate/Function Select */
  { OUT, GRREG, 4, GRVAL, 0x00 },  /* Read Plane */
  { OUT, GRREG, 5, GRVAL, 0x10 },  /* Access Mode */
  { OUT, GRREG, 6, GRVAL, 0x0e },  /* b0:Graphics b1:CGA B2-3:VRAM Addr */
  { OUT, GRREG, 7, GRVAL, 0x00 },  /* Color Don't Care Plane */
  { OUT, GRREG, 8, GRVAL, 0xff },  /* Bit Mask */
  ...
  { OUT, PALREG, 14, PALREG, 0x3e },
  { OUT, PALREG, 15, PALREG, 0x3f },
  { OUT, PALREG, 16, PALREG, 0x0c },
  { OUT, PALREG, 17, PALREG, 0x00 },
  { OUT, PALREG, 18, PALREG, 0x0f },
  { OUT, PALREG, 19, PALREG, 0x08 },
  { OUT, PALREG, 20, PALREG, 0x00 },
...
ega_hwterm(void)
{
...
#else
  {
	  ...
	  int 		ch;
	  int 		row;
      unsigned int font_chars=256;
      unsigned int font_char_height=16;
      unsigned int ram_scan_lines=32;

      out_word(SEQREG, 0x0604); /* vram 128KB sequential addressing */
      out_word(GRREG,  0x0005); /* disable odd-even addressing */
      out_word(GRREG,  0x0c06); /* vram b8000- & CGA:Non Graph:Non */
      out_word(GRREG,  0x0204); /* select map 2 for CPU reads */
      out_word(SEQREG, 0x0402); /* cpu writes only to map 2 */
      out_word(GRREG,  0x0000); /* set/reset latch */
      out_word(GRREG,  0x0001); /* enable set/reset latch */
      out_word(GRREG,  0x0002); /* color compare latch */
      out_word(GRREG,  0x0003); /* pset mode */
      out_word(GRREG,  0x0007); /* color don't care */
      out_word(GRREG,  0xff08); /* bit mask */

	  srcoffset =  g_8x16_font;
	  destoffset = 0xb8000;//EGA_BASE;
	  ...
	  for (ch = 0; ch < font_chars; ch++) {
		...
		destoffset += (ram_scan_lines - font_char_height);
	  }
  ...
  for (; rp->action != DONE; rp++) {
	switch (rp->action) {
	    case IN:
			...
			break;
	    case OUT:
			outb(rp->data1,rp->port1);
			if (rp->port2) {
				outb(rp->data2,rp->port2);
            }
			break;
	}
  }
...
setmode(int mode)
{
  if (mode > MWMODE_MAX)
  	return;
  outb(DATA_ROTATE, GRREG);
  outb(mode_table[mode], GRVAL);
}
