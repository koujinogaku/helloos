...
start(int ac, char **av)
{
	...
	w1 = GrNewWindowEx(
		...
		GR_WM_PROPS_CLOSEBOX, (GR_CHAR*)"nxclock", GR_ROOT_WINDOW_ID, 
...
do_clock(void)
{
	...
	struct clock_datetime_set timeset;

	clock_get_date(&timeset);
	if (timeset.tm_sec == then_sec) {
		return;
	}
	then_sec = timeset.tm_sec;
	minutes = timeset.tm_min * 6;
	sec = timeset.tm_sec * 6;
	hour = timeset.tm_hour;
	if (hour >= 12) {
		hour -= 12;
	}
